...
const { Button, Header, Section } = require("../../Base");
...
const listBackendTasks = async () => {
  let data = dataCache.getSync("listBackendTasks") || {};
  let newTaskModalShown = dataCache.getSync("newTaskModalShown") || false;
  log(data);
  ...
  return `${await Header({    children: async () => `<h2>Watch Tasks:</h2>${await Button(newTaskButton)}`,    options: {},  })}
            <content>
              ${tasks
                .map(
